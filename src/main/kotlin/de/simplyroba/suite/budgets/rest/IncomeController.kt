...
class IncomeController(private val incomeService: IncomeService) {
  ...
  fun getIncomeListByYearAndMonth(@PathVariable year: Int, @PathVariable month: Int): Flux<Income> =
    incomeService.findAllByYearAndMonth(year, month)

  @GetMapping("/{id}")
  fun getIncomeById(@PathVariable id: Long): Mono<Income> = incomeService.findById(id)

  @PostMapping
  @ResponseStatus(HttpStatus.CREATED)
  fun createIncome(@RequestBody income: IncomeCreate): Mono<Income> =
    incomeService.createIncome(income)

  @PutMapping("/{id}")
  fun updateIncome(@PathVariable id: Long, @RequestBody income: IncomeUpdate): Mono<Income> =
    incomeService.updateIncome(id, income)

  @DeleteMapping("/{id}")
  @ResponseStatus(HttpStatus.NO_CONTENT)
  fun deleteIncome(@PathVariable id: Long): Mono<Void> = incomeService.deleteIncome(id)
}
